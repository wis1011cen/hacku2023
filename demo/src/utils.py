...
def arm_operation(landmark_dict, annotated_frame, appliance_dict):
    ...
    if l_flag == 1 and r_flag == 1:
        if landmark_dict['l_visibility'] > landmark_dict['r_visibility']:
    ...
    cv2.putText(annotated_frame,  f"L:{landmark_dict['l_visibility']:.2f}", (0, 100), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
    cv2.putText(annotated_frame,  f"R:{landmark_dict['r_visibility']:.2f}", (0, 150), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
    ...
    if pre_appliance_name is not None:
        ...
        ex_pos = landmark_dict['r_wrist'] + 100 * (landmark_dict['r_wrist'] - landmark_dict['r_elbow'])    #腕を伸ばした先
...
def l_ir_operation(annotated_frame, wrist_pos, appliance_dict, elbow_pos):
    ...
    extended_pos = wrist_pos + 100 * (wrist_pos - elbow_pos)    #腕を伸ばした先
    color = (255, 0, 0)
