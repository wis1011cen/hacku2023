...
def pose_detector_callback(result, output_frame, timestamp):
    ...
    if not flip_flag:
        ...
        landmark_names = ('r_shoulder', 'l_shoulder', 'r_elbow', 'l_elbow', 'r_wrist', 'l_wrist')
    ...
    for idx in range(len(pose_landmarks_list)):
        ...
        for i , landmark_name in enumerate(landmark_names, 11):
            ...
            if not flip_flag:
                ...
                landmark_dict['l_visibility'] = pose_landmarks[16].visibility
                landmark_dict['r_visibility'] = pose_landmarks[15].visibility
...
def main(input_flip_flag):
    global annotated_frame, appliance_dict, start_time_dict, flip_flag
    
    flip_flag = input_flip_flag
    ...
    pose_detector_options = vision.PoseLandmarkerOptions(base_options=python.BaseOptions(model_asset_path='models/pose_landmarker_lite.task'),
                                                         ...
                                                         num_poses=1,
                                                         min_pose_detection_confidence=utils.MIN_POSE_DETECTION_CONFIDENCE,
                                                         min_pose_presence_confidence=utils.MIN_POSE_PRESENCE_CONFIDENCE, 
    ...
    # csvファイルから読み込む場合 
        ...
        appliance_dict = {'tv':(500*SCALE, 260*SCALE, 140*SCALE, 100*SCALE)}
    """
    appliance_dict = dict()
    with open('src/roi.csv') as f:
        reader = csv.reader(f)
    ...
    if flip_flag:
        for name, (x, y, w, h) in appliance_dict.items():
            appliance_dict[name] = (WIDTH-x-w, y, w, h)
