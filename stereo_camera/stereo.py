...
def depth_estimate(framel, framer, map1_l, map2_l, map1_r, map2_r, wls_filter, left_matcher, right_matcher):
    ...
    filtered_frame = wls_filter.filter(displ, framel, None, dispr)
    # print(filtered_frame[240,320])
    ...
    max_idx = np.unravel_index(np.argmax(filtered_frame), filtered_frame.shape)
    min_idx = np.unravel_index(np.argmin(filtered_frame), filtered_frame.shape)
    # print(idx)
    # print(max_idx, min_idx)
    
    THRESHOLD = -1000
    filtered_frame = np.where(filtered_frame < THRESHOLD, THRESHOLD, filtered_frame)
    filtered_frame = np.where(filtered_frame > 1000, 1000, filtered_frame)
    
    filtered_frame = cv2.normalize(
        src=filtered_frame, dst=filtered_frame, beta=0, alpha=255, norm_type=cv2.NORM_MINMAX)
    ...
    return framel_gray, framer_gray, filtered_frame
...
def test():
    ...
    map1_l, map2_l, map1_r, map2_r, wls_filter, left_matcher, right_matcher = load_caliblation_data()
